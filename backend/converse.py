...
# Function to rank US companies based on extracted values
...
def model():
    ...
    # Get the company name from request
    ...
    # Generate competitors for the company
    user_message = (
        f"Give me the top 5 competitors of {company}. "
        f"Format it as: company,competitor1,competitor2,competitor3,competitor4,competitor5."
    )
    ...
    try:
        ...
        # Remove the first word if it's "company"
        competitors_list = company_competitors.split(",")
        if competitors_list and competitors_list[0].lower() == "company":
            competitors_list = competitors_list[1:]
        company_competitors = ",".join(competitors_list)
    ...
    prompts = [
        ...
        f"Identify one specific new product that's trending and provide feedback: {raw_data + processed_data}",
    ]
